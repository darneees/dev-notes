...
import { IconoirProvider, Clock, Trash } from 'iconoir-react'
    ...
    return (
        <Dialog.Root>
            <Dialog.Trigger className='rounded-md text-left outline-none flex flex-col bg-slate-800 p-5 gap-3 overflow-hidden relative hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400'>
                <span className='flex row-auto gap-2 text-sm font-medium text-slate-200'>
                    {formatDistanceToNow(note.date, { locale: ptBR, addSuffix: true })}

                    <IconoirProvider>
                        <Clock className='text-blue-400 w-6' />
                    </IconoirProvider>
                </span>
            ...
            <Dialog.Portal>
                ...
                <Dialog.Content className='fixed inset-0 md:inset-auto overflow-hidden md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2  md:max-w-[640px] w-full md:h-[60vh] bg-slate-700 md:rounded-md flex flex-col outline-none'>
                    ...
                    <div className='flex flex-1 flex-col gap-3 p-5'>
                        <span className='flex row-auto gap-2 text-sm font-medium text-slate-300'>
                            {formatDistanceToNow(note.date, { locale: ptBR, addSuffix: true })}

                            <IconoirProvider>
                                <Clock className='text-blue-300 w-6' />
                            </IconoirProvider>
                        </span>
                    ...
                    <button type='button' 
                            ...
                            className='flex justify-center items-center gap-1 w-full bg-gray-600 py-4 text-center text-sm text-slate-300 outline-none font-medium group'
                    >
